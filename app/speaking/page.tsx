export default function SpeakingPage() {
  ...
  return (
    <main>
      ...
      <section className="section"><div className="section-heading"><p className="kicker">SPEAKING</p><h1>Technical talks for people who actually build things.</h1><p>Talks and live education about APIs, integrations, reliability, developer experience, AI, and the lessons hidden inside failure.</p></div><div className="talk-list">{talks.map((talk, index) => <article key={talk.title}><span>0{index + 1}</span><div><h2>{talk.title}</h2><p>{talk.copy}</p></div></article>)}</div></section>
      <section className="section community-section"><div className="community-card"><p className="kicker">CURRENT GOAL</p><h2>Take Sugar Bytes to the stage.</h2><p>The first target is a local developer meetup talk, with PyATL as an immediate speaking opportunity and future talks expanding into API, DevRel, and developer-tool communities.</p><div className="tag-row"><span>Meetups</span><span>Webinars</span><span>Workshops</span><span>Conferences</span></div></div></section>
