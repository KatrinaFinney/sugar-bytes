  ...
  return (
    <main>
      ...
      <section className="section muted-section">
        <div className="section-heading"><p className="kicker">LATEST TEA</p><h2>{published.length ? "Fresh from the community." : "A field series built to grow in public."}</h2><p>{published.length ? "Quick takes from real developer events and hallway conversations." : "Developer Tea captures the kind of answers that rarely make it into polished product messaging. As Sugar Bytes gets into more rooms, this archive will become a running record of what developers are actually thinking."}</p></div>
        {published.length ? <div className="program-grid">{published.map((episode) => <EditorialCard key={episode.slug} eyebrow="DEVELOPER TEA" title={episode.title} copy={episode.dek} meta={episode.event || episode.location} href={`/developer-tea/${episode.slug}`} />)}</div> : <div className="empty-state"><p>The format is simple on purpose: one strong question, several honest answers, and enough context to spot where the community agrees, disagrees, or surprises us.</p></div>}
      </section>
